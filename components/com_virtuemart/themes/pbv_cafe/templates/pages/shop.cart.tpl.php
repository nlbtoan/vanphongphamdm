    ...
    if( $continue_link != '') {
		?>
		 <a href="<?php echo $continue_link ?>" class="continue_link">
		 	<?php echo vmCommonHTML::imageTag(VM_THEMEURL."images/btn_KeepShopping.gif", $VM_LANG->_('PHPSHOP_CONTINUE_SHOPPING')); ?></a>&nbsp;&nbsp; 
		 </a>
		<?php
    }
   ...
   if (!defined('_MIN_POV_REACHED')) { ?>
       ...
       <?php
   		...
   		if( $this->get_cfg('useGreyBoxOnCheckout', 1)) {
   			$text=  vmCommonHTML::imageTag(VM_THEMEURL."images/btn_ProceedtoCheckout.gif", 'Check out');
