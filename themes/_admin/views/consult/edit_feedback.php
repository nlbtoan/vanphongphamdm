<?php echo doctype(); ?>

<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title><?php echo lang('tieu_de') ?></title>
<?php $this->load->view('head') ?>

<!-- LOAD JS -->
<script type="text/javascript" src="<?php echo site_url('assets/jquery-ui-1.8.2.custom.min.js')?>"></script>
<script type="text/javascript" src="<?php echo site_url('assets/jquery.easy-confirm-dialog.js')?>"></script>
<script type="text/javascript" src="<?php echo site_url('assets/tinymce.jquery/jquery.tinymce.js')?>"></script>
<script type="text/javascript" src="<?php echo site_url('assets/tinymce.jquery/tiny_mce.js')?>"></script>
<!-- LOAD CSS -->
<?php load_css('assets/message/screen.css') ?>

<script type="text/javascript">
$(document).ready(function(){
	
	$('#message, .notice').show().delay(10000).fadeOut('slow');

	$('textarea').tinymce({
		// General options
		theme : "advanced",
		plugins : "pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,inlinepopups,autosave",

		// Theme options
		theme_advanced_buttons1 : "bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,formatselect,fontselect,fontsizeselect,|,forecolor,backcolor",
		theme_advanced_buttons2 : "bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,insertdate,inserttime,|,sub,sup,|,charmap,emotions,|,link,unlink",
		theme_advanced_buttons3 : "",
		theme_advanced_toolbar_location : "top",
		theme_advanced_toolbar_align : "left"
	});
});
</script>
</head>

<body>	
    <div id="container">    	   
        <!-- ########################## HEADER ########################## -->
		<div id="header">
       	<?php $this->load->view('header') ?>                
        </div>          
		<!-- ########################## END HEADER ###################### -->
        
       <!-- ########################## LEFT ########################## -->
        <div id="left" class="panel">        	            
			<?php $this->load->view('leftsiderbar') ?>                                                    
        </div>        
        <!-- ########################## END LEFT ###################### -->
        
        <!-- ########################## WRAPPER ########################## -->
        <div id="wrapper" class="panel">
            <div class="panel_top"><div></div></div>            
            <div class="panel_left">
                <div class="panel_right">
                	<!-- ########################## ICON & BREADCRUMB ########################## --> 
				    <p class="float_l"><img src="<?php echo theme_url('styles/default/images/icons/user.gif') ?>" /></p>					
				    <p class="float_l">
				        <span class="large">Thêm menu con</span><br />
				        <span class="quiet">Quản lý menu</span>
				    </p>
				    <div class="clear"></div>
				    
				    <!-- ########################## TOOLBAR ########################## -->
				    <div class="toolbar">
				    	<button class="negative" type="button" onclick="window.location = '<?php echo $back_url?>'"><img src="<?php echo theme_url('styles/default/images/icons/back.png')?>"/><br/><?php echo lang('layout_toolbar_cancel') ?></button>
				    	<button class="positive" type="button"><img src="<?php echo theme_url('styles/default/images/icons/email-icon.png')?>"/><br/>Trả lời</button>
				    </div>
				    
				<!-- ########################## CONTENT ########################## -->
					
					<fieldset>
						<legend>Thông tin chung</legend>
						<table class="editing">
							<tr>
			            		<th><?php echo lang('your_name');?>  : </th>
			            		<td><?php if ( !empty($restore_data) ) echo $restore_data['fullname']; ?></td>
			            	</tr>
			            	<tr>
			            		<th><?php echo lang('email');?>  : </th>
			            		<td><?php if ( !empty($restore_data) ) echo $restore_data['email']; ?></td>
			            	</tr>
			            	<tr>
			            		<th><?php echo lang('table_th_date');?>  : </th>
			            		<td><?php if ( !empty($restore_data) ) echo date('d/m/Y',$restore_data['date']) ?></td>
			            	</tr>
			            	<tr>
			            		<th><?php echo lang('title_fb');?> : </th>
			            		<td><?php if ( !empty($restore_data) ) echo $restore_data['title']; ?></td>
			            	</tr>
			            	<tr>
			            		<th><?php echo lang('content_fb');?> : </th>
			            		<td><?php if ( !empty($restore_data) ) echo $restore_data['content']; ?></td>
			            	</tr>
						</table>
					</fieldset>
					
									    
				    <!-- ########################## MESSAGE ########################## -->
				    <div id="message">
				    	<?php if( !empty($message) ) echo $message;?>
				    </div>
				    <div class="clear"></div>
					
					<fieldset>
						<legend>Trả lời</legend>
						<?php echo form_open("admin/consult/edit_feedback/".$restore_data['id']);?>
						<input type="hidden" name="id" value="<?php if ( !empty($restore_data) ) echo $restore_data['id']; ?>"/>
						<table class="editing">
			            	<tr>
			            		<th><?php echo lang('mail_to')?> : </th>
			            		<td><input type="text" size="80" name="mail_to" value="<?php if ( !empty($restore_data) ) echo $restore_data['email']; ?>"></td>
			            	</tr>
			            	<tr>
			            		<th><?php echo lang('subject')?> : </th>
			            		<td><input type="text" size="80" name="subject" value="<?php if ( !empty($restore_data) ) echo 'Re: '.$restore_data['title']; ?>"></td>
			            	</tr>
						</table>
        					<textarea rows="15" cols="80" name="mail_content">
								<br/>
								<br/>
								<br/>
								<div>
								<?php if ( !empty($restore_data) ) echo date('d/m/Y',$restore_data['date']) ?> 
								<?php if ( !empty($restore_data) ) echo $restore_data['fullname']; ?> 
								<?php if ( !empty($restore_data) ) echo ' < '.$restore_data['email'].' > '; ?>
								<p style="padding:10px; border-left:1px solid #CCC;">
									<?php if ( !empty($restore_data) ) echo $restore_data['content']; ?>
								</p>
								</div>
							</textarea>
							<button class="positive" type="submit">Gửi</button>
       					<?php echo form_close();?>
					</fieldset>
				<!-- ########################## END CONTENT ########################## -->  				  	    				   				    				                                                                                                                                                                            
                </div>
            </div>
            <div class="panel_bottom"><div></div></div>
        </div><!--/wrapper-->

        <!-- ########################## END WRAPPER ###################### -->
        <div class="clear"></div>        
    </div><!--/containner-->
</body>
</html>