...
var toolbar = {
	...
	_delete : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/tour/delete");
				var array_id = [];
	...
	_publish : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/tour/update");
				var array_id = [];
...
$(document).ready(function(){
	...
	*/
	...
	$('.hot').click(function(e){
		...
		if( $(this).hasClass('tick') )
		{
			$.post(url, {id : data_id, active : 0, current_url : $('#current_url').val(), action : 'check_hot'} );
		}
		else if( $(this).hasClass('untick') )
		{
			$.post(url, {id : data_id, active : 1, current_url : $('#current_url').val(), action : 'check_hot'} );
		}	
	});
    ...
    <div id="container">    	   
       ...
       <!-- ########################## LEFT ########################## -->
        ...
        <div id="wrapper" class="panel">
        	<form action="<?php echo site_url("admin/tour/manage/$current_cat");?>" method="post" id="adminForm">        	
            ...
            <div class="panel_left">
                <div class="panel_right">
				    ...
				    <div class="toolbar">
				    	...
				    	<button class="positive" type="button" title="<?php echo lang('action_add');?>"  onclick="toolbar._insert('<?php echo site_url('admin/tour/create');?>')"><img src="<?php echo theme_url('styles/default/images/icons/insert_row.png')?>"/><br/><?php echo lang('layout_toolbar_insert') ?></button>
				...
				<table class="listing" id="listing">
			        ...
			        <tr>
			            ...
			            <th><?php echo lang('table_th_title') ?></th>
			            <th><?php echo lang('table_th_hot') ?></th>
					...
					<?php for ($i=0; $i < count($data); $i++ ): ?>
						<tr>
							...
							<td style="text-align: left; padding-left: 5px;"> <a href="<?php echo site_url('admin/tour/edit/' . $data[$i]['id'])?>"  style="white-space:nowrap; overflow:hidden; width:480px;" title="<?php echo lang('action_see_and_edit');?>"><?php echo $data[$i]['title']?><span style="color:#777;"> - <?php echo date('d/m/Y', $data[$i]['date'])?></span></a></td>
							<td> <a href="" class="icon hot <?php if($data[$i]['is_hot']) echo('tick'); else echo('untick');?>" rel="<?php echo $data[$i]['id']?>" title="<?php if($data[$i]['is_hot']) echo( lang('action_lock') ); else echo(lang('action_open'));?>"></a> </td>
							...
							<td> <a href="<?php echo site_url('admin/tour/edit/' . $data[$i]['id'])?>" class="icon edit" rel="<?php echo $data[$i]['id']?>" title="<?php echo lang('action_edit');?>"></a></td>
						</tr>
					<?php endfor;?>
					<?php endif;?>
			    </tbody>
