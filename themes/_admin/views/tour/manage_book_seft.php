...
var toolbar = {
	...
	_delete : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/tour/delete_book");
				var array_id = [];
	...
	_publish : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/tour/update_book");
				var array_id = [];
    ...
    <div id="container">    	   
       ...
       <!-- ########################## LEFT ########################## -->
        ...
        <div id="wrapper" class="panel">
        	<form action="<?php echo site_url("admin/tour/manage_book");?>" method="post" id="adminForm">        	
				...
				<table class="listing" id="listing">
					...
					<?php for ($i=0; $i < count($data); $i++ ): ?>
						<tr>
							...
							<td> <a href="<?php echo site_url('admin/tour/edit_book_seft/' . $data[$i]['id'])?>" class="icon menu pad_left"  style="white-space:nowrap; overflow:hidden; width:470px; <?php if(!$data[$i]['is_read']) echo 'font-weight:bold;'; ?>" title="<?php echo lang('action_see_and_edit');?>"><?php echo date('h:i:s - d/m/Y', $data[$i]['date'])?></a></td>
							...
							<td> <a href="<?php echo site_url('admin/tour/edit_book_seft/' . $data[$i]['id'])?>" class="icon edit" rel="<?php echo $data[$i]['id']?>" title="<?php echo lang('action_edit');?>"></a></td>
						</tr>
					<?php endfor;?>
					<?php endif;?>
			    </tbody>
