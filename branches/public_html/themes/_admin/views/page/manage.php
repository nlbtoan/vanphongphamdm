...
var toolbar = {
	...
	_delete : function()
	{
		...
		{
			var answer = confirm('Bạn chắc chắn muốn xóa trang');
			if(answer)
			{
				var url = site_url("admin/page/delete_page");
				var array_id = [];
	...
	_publish : function()
	{
		...
		{
			var answer = confirm('Bạn chắc chắn muốn mở trang');
			if(answer)
			{
				var url = site_url("admin/page/update_page");
				var array_id = [];
...
$(document).ready(function(){
	...
	*/
	$('.active').click(function(e){
		...
		{
			$.post(url, {id : page_id, active : 0, current_url : $('#current_url').val(), action : 'publish'} );
		}
		else if( $(this).hasClass('untick') )
		{
			$.post(url, {id : page_id, active : 1, current_url : $('#current_url').val(), action : 'publish'} );
		}	
	});
	...
	*/
	$('.delete').click(function(e){
		...
		if(answer)
		{
			...
			$.post(url, {id : page_id, current_url : $('#current_url').val(), action : 'delete'} );
		}
	});
    ...
    <div id="container">    	   
       ...
       <!-- ########################## LEFT ########################## -->
        ...
        <div id="wrapper" class="panel">
        	<form action="<?php echo site_url("admin/page/manage");?>" method="post" id="adminForm">        	
            ...
            <div class="panel_left">
                <div class="panel_right">
				    ...
				    <!-- ########################## TOOLBAR ########################## -->
				    <div class="toolbar">
				    	<button class="positive" type="button" onclick="toolbar._insert('<?php echo site_url('admin/page/create_page');?>')"><img src="<?php echo theme_url('styles/default/images/icons/insert_row.png')?>"/><br/><?php echo lang('layout_toolbar_insert') ?></button>
				...
				<table class="listing" id="listing">
			        ...
			        <tr>
			            ...
			            <th><?php echo lang('table_th_title_page') ?></th>
					...
					<?php for ($i=0; $i < count($pages); $i++ ): ?>
						<tr>
							<td> <input type="checkbox" value="<?php echo $pages[$i]['id']?>" class="checkbox"/> </td>
							<td> <?php echo ( $i + 1 )?> </td>
							<td> <?php echo $pages[$i]['id']?> </td>
							<td> <a href="<?php echo site_url('admin/page/edit_page/' . $pages[$i]['id'])?>" class="icon menu pad_left"><?php echo $pages[$i]['title']?> </a></td>
							<td> <a href="" class="icon active <?php if($pages[$i]['active']) echo('tick'); else echo('untick');?>" rel="<?php echo $pages[$i]['id']?>"></a> </td>
							<td> <a href="" class="icon delete" rel="<?php echo $pages[$i]['id']?>"></a></td>
							<td> <a href="<?php echo site_url('admin/page/edit_page/' . $pages[$i]['id'])?>" class="icon edit" rel="<?php echo $pages[$i]['id']?>"></a></td>
						</tr>
					<?php endfor;?>
					<?php endif;?>
			    </tbody>
