	...
	$('a.remove_row').bind('click', event_remove);
	 ...
	 $('#add_client').bind('click', function(e){
		...
		html += '<td>';
			...
			html += '</select>';
		html += '</td>';
		html += '<td><input type="text" name="txt_passport_i[]" autocomplete="off" size="10"/></td>';
		html += '<td><input type="text" name="txt_date_issue_i[]" autocomplete="off" size="8" readonly="readonly" style="background:#FFF; border:1px solid #7F9DB9; height:18px;"/></td>';
		html += '<td><input type="text" name="txt_date_expiry_i[]" autocomplete="off" size="8" readonly="readonly" style="background:#FFF; border:1px solid #7F9DB9; height:18px;"/></td>';
		html += '<td>';
	 ...
	 $("input[name*=txt_birthday], input[name*=txt_date_issue], input[name*=txt_date_expiry]").datepicker({
		 changeMonth: true,
		 changeYear: true,
		 minDate: new Date(1990, 0, 1),
		 maxDate: 'today'
	 ...
	 $('#btn_submit').click(function(e){
		...
		e.preventDefault();
	 	...
	 	$('input[name*=txt_name]').each(function(idx){
	 		if ( $.trim($(this).val()).length < 1 || $.trim($("input[name*=txt_passport]").eq(idx).val()).length < 1 || $.trim($("input[name*=txt_passport]").eq(idx).val())==0){
				flag = false;
	 		}	
		});
			            		...
			            		<td colspan="2">
			            			...
			            			<table id="list_p" cellspacing="1">
										...
										<tr>
											...
											<td><input name="txt_passport[]" value="<?php echo $item['passport'] ?>" type="text"  autocomplete="off" size="10" autocomplete="off"/></td>
											<td><input name="txt_date_issue[]" value="<?php echo $item['date_issue'] != 0 ? date('m/d/Y', $item['date_issue']) : "" ; ?>" type="text"  autocomplete="off" size="8" readonly="readonly" style="background:#FFF; border:1px solid #7F9DB9; height:18px;" autocomplete="off"/></td>
											<td><input name="txt_date_expiry[]" value="<?php echo $item['date_expiry'] != 0 ? date('m/d/Y', $item['date_expiry']) : "" ; ?>" type="text"  autocomplete="off" size="8" readonly="readonly" style="background:#FFF; border:1px solid #7F9DB9; height:18px;" autocomplete="off"/></td>
											<td>
