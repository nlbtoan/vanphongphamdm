...
var toolbar = {
	...
	_delete : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/tour/delete_region");
				var array_id = [];
	...
	_publish : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/tour/update_region");
				var array_id = [];
    ...
    <div id="container">    	   
       ...
       <!-- ########################## LEFT ########################## -->
        ...
        <div id="wrapper" class="panel">
        	<form action="<?php echo site_url("admin/tour/manage_region");?>" method="post" id="adminForm">        	
            ...
            <div class="panel_left">
                <div class="panel_right">
				    ...
				    <p class="float_l">
				        <span class="large"><?php echo lang('title_manage_region') ?></span><br />
				    ...
				    <!-- ########################## TOOLBAR ########################## -->
				    <div class="toolbar">
				    	<button class="positive" type="button" title="<?php echo lang('action_add');?>"  onclick="toolbar._insert('<?php echo site_url('admin/tour/create_region');?>')"><img src="<?php echo theme_url('styles/default/images/icons/insert_row.png')?>"/><br/><?php echo lang('layout_toolbar_insert') ?></button>
				...
				<table class="listing" id="listing">
			        ...
			        <tr>
			            ...
			            <th><?php echo lang('table_th_name_region') ?></th>
			            <th><?php echo lang('table_th_alias_region') ?></th>
					...
					<?php foreach( $data as $k => $item ):?>
						<tr>
							...
							<td> 
								<a href="<?php echo site_url('admin/tour/edit_region/' . $item['data']['id'])?>" title="<?php echo lang('action_see_and_edit');?>" class="icon menu pad_left">
									<?php 
				        				if($item['lv'] > 0){
					        				for($i = 0; $i < $item['lv']; $i++)
					        				{
							...
							<td> <a href="" class="icon active <?php if($item['data']['active']) echo('tick'); else echo('untick');?>" rel="<?php echo $item['data']['id']?>" title="<?php if($item['data']['active']) echo( lang('action_lock') ); else echo(lang('action_open'));?>"></a> </td>
							<td> <a href="" class="icon delete" rel="<?php echo $item['data']['id']?>" title="<?php echo lang('action_delete');?>"></a></td>
							<td> <a href="<?php echo site_url('admin/tour/edit_region/' . $item['data']['id'])?>" class="icon edit" rel="<?php echo $item['data']['id']?>" title="<?php echo lang('action_edit');?>"></a></td>
						</tr>
					<?php endforeach;?>
					<?php endif;?>
			    </tbody>
