...
var toolbar = {
	...
	_delete : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/vehicle/delete_book");
				var array_id = [];
	...
	_publish : function()
	{
		...
		else
		{
			...
			if(answer)
			{
				var url = site_url("admin/vehicle/update_book");
				var array_id = [];
    ...
    <div id="container">    	   
       ...
       <!-- ########################## LEFT ########################## -->
        ...
        <div id="wrapper" class="panel">
        	<form action="<?php echo site_url("admin/vehicle/manage_book");?>" method="post" id="adminForm">        	
            ...
            <div class="panel_left">
				...
				<table class="listing" id="listing">
			        ...
			        <tr>
			            ...
			            <th><?php echo lang('table_th_date_book') ?></th>
			            <th><?php echo lang('table_th_check') ?></th>
					...
					<?php for ($i=0; $i < count($data); $i++ ): ?>
						<tr>
							...
							<td> <a href="<?php echo site_url('admin/vehicle/edit_book/' . $data[$i]['id'])?>" class="icon menu pad_left"  style="white-space:nowrap; overflow:hidden; width:470px; <?php if(!$data[$i]['is_read']) echo 'font-weight:bold;'; ?>" title="<?php echo lang('action_see_and_edit');?>"><?php echo date('h:i:s - d/m/Y', $data[$i]['date'])?></a></td>
							<td> <a href="" class="icon active <?php if($data[$i]['is_checked']) echo('tick'); else echo('untick');?>" rel="<?php echo $data[$i]['id']?>" title="<?php if($data[$i]['is_checked']) echo( lang('action_lock') ); else echo(lang('action_open'));?>"></a> </td>
							<td> <a href="" class="icon delete" rel="<?php echo $data[$i]['id']?>" title="<?php echo lang('action_delete');?>"></a></td>
							<td> <a href="<?php echo site_url('admin/vehicle/edit_book/' . $data[$i]['id'])?>" class="icon edit" rel="<?php echo $data[$i]['id']?>" title="<?php echo lang('action_edit');?>"></a></td>
						</tr>
					<?php endfor;?>
					<?php endif;?>
			    </tbody>
