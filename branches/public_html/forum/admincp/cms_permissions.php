...
function saveData()
{
	...
	if ($vbulletin->GPC_exists['count'] AND $vbulletin->GPC_exists['sectionid'])
	{
		...
		for ($i = 1; $i <= intval($vbulletin->GPC['count']) ; $i++)
		{
			...
			{
				$perm = (isset($_POST["cb_1_$i" ]) ? 1 : 0)
						+ (isset($_POST["cb_2_$i" ]) ? 32 : 0)
						+ (isset($_POST["cb_3_$i" ]) ? 2 : 0)
						+ (isset($_POST["cb_4_$i" ]) ? 4 : 0)
						+ (isset($_POST["cb_5_$i" ]) ? 8 : 0)
						+ (isset($_POST["cb_6_$i" ]) ? 16 : 0);
	...
	if ($rst = $vbulletin->db->query_read($sql))
	{
		...
		while($row = $vbulletin->db->fetch_array($rst))
		{
			if (intval($row['nodeid']) == intval($row['parentid']) )
			{
				...
				$stack[$count][] = "<a href=\"javascript:" . (intval($row['parentnode']) ? 'showRemove=true;' : 'showRemove=false;')
...
function getSectionList($orderby = 1)
{
	...
	if ($rst = $vbulletin->db->query_read($sql))
	{
		...
		while($row = $vbulletin->db->fetch_array($rst))
		{
			...
			if (intval($row['nodeid']) == intval($row['parentid']) )
			{
				$stack[$count][] = '<a href="javascript: setSection(' . $row['nodeid'] . '); return false;" onclick="javascript:void setSection('
