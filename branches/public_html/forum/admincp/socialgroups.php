...
define('CVS_REVISION', '$RCSfile$ - $Revision: 35024 $');
...
if ($_REQUEST['do'] == 'dosearch')
{
	...
	if (!empty($groups))
	{
		...
		foreach ($groups AS $group)
		{
			...
			$cell .= '</span>
				<div style="text-align: ' . vB_Template_Runtime::fetchStyleVar('left') . '"><a href="../group.php?' . $vbulletin->session->vars['sessionurl']. 'groupid=' . $group['groupid'] . '" target="group">' . $group['name'] . '</a></div>
