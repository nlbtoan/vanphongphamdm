...
if ($do == 'index')
{
	...
	$output .= "<p class=\"largefont\">$vbphrase[view_full_version]: <a href=\"" . $vbulletin->options['bburl'] . '/' .
		$vbulletin->options['forumhome'] . '.php' .
		$vbulletin->session->vars['sessionurl_q'] . '">' . $vbulletin->options['bbtitle'] . "</a></p>\n";
...
if ($do == 'forum')
{
	...
	$output .= "<p class=\"largefont\">$vbphrase[view_full_version] : <a href=\"" .
		$vbulletin->options['bburl'] . '/' . fetch_seo_url('forum', $foruminfo) .
	...
	if ($foruminfo['cancontainthreads'])
	{
		...
		while ($thread = $db->fetch_array($threads))
		{
			...
			{
				$output .= "\t<li>$thread[prefix_plain_html] <a href=\"" . $vbulletin->options['bburl'] .
					'/' . fetch_seo_url('thread', $thread) . "\">$thread[title]</a></li>\n";
			}
		}
		$output .= "</ol>\n</div>\n";
...
if ($do == 'thread')
{
	if (!$vbulletin->options['archive_threadtype'])
	{
		...
		exec_header_redirect($vbulletin->options['bburl'] . '/' . fetch_seo_url('thread|js', $threadinfo));
	}
	...
	$output .= "<p class=\"largefont\">$vbphrase[view_full_version] : "
		...
		. "<a href=\"" . $vbulletin->options['bburl'] . '/' . fetch_seo_url('thread', $threadinfo)
