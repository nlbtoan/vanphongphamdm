...
if ($_REQUEST['do'] == 'register')
{
	...
	while ($profilefield = $db->fetch_array($profilefields))
	{
		...
		else if (isset($vbulletin->GPC['userfield']["$profilefieldname"]))
		{
		...
		if ($profilefield['type'] == 'input')
		{
			...
			{
				$profilefield['currentvalue'] = htmlspecialchars_uni($vbulletin->userinfo["$profilefieldname"]);
			}
