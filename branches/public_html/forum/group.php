...
if ($_REQUEST['do'] == 'view')
{
	...
	if ($show['groupdiscussions'])
	{
		...
		$orderlinks = array(
			'title' => $sorturl . '&amp;sort=title' . ($sortfield == 'title' ? '&amp;order=' . $oppositesort : ''),
			'author' => $sorturl . '&amp;sort=author' . ($sortfield == 'author' ? '&amp;order=' . $oppositesort : ''),
...
if ($_POST['do'] == 'updatepictures')
{
	...
	while ($picture = $db->fetch_array($picture_sql))
	{
		...
		{
			if ($picture['state'] == 'moderation' AND can_moderate(0, 'canmoderategrouppictures') AND $vbulletin->GPC['pictures']["$picture[attachmentid]"]['approve'])
			{
	...
	if ($vbulletin->GPC['frompicture'] AND sizeof($attachmentids) == 1 AND !$deleted_picture)
	{
		...
		$vbulletin->url = 'group.php?' . $vbulletin->session->vars['sessionurl'] . "groupid=$group[groupid]&amp;attachmentid=$attachmentid";
	}
	else
	{
		$vbulletin->url = 'group.php?' . $vbulletin->session->vars['sessionurl'] . "groupid=$group[groupid]";
...
if ($_REQUEST['do'] == 'editpictures')
{
	...
	$display = $db->query_first("
		...
		WHERE
			...
			" . (!can_moderate(0, 'canmoderategrouppictures') ? "AND (a.state = 'visible' OR a.userid = " . $vbulletin->userinfo['userid'] . ")" : "") . "
	");
