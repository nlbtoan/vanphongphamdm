 ...
 * @see vB_Types
 *
 * @version $Revision: 34659 $
 * @since $Date: 2009-12-22 14:54:33 -0800 (Tue, 22 Dec 2009) $
...
class vBCms_Types extends vB_Types
{
	...
	public function assertWidgetType($widgettype, vB_Exception $e = null)
	{
		...
		{
			throw ($e ? $e : new vB_Exception_Warning('Invalid widgettype \'' . htmlspecialchars(print_r($widgettype, 1)) . '\''));
		}
	...
	public function getWidgetTypePackage($widgettype)
	{
		if (!($id = $this->getWidgetTypeID($widgettype)))
		{
			throw (new vB_Exception_Warning('Trying to get package class from invalid widgettype \'' . htmlspecialchars($widgettype) . '\''));
		}
	...
	public function getWidgetTypeClass($widgettype)
	{
		if (!($id = $this->getWidgetTypeID($widgettype)))
		{
			throw (new vB_Exception_Warning('Trying to get widgettype class from invalid widgettype \'' . htmlspecialchars($widgettype) . '\''));
		}
	...
	public function getWidgetTypeTitle($widgettype)
	{
		if (!($id = $this->getWidgetTypeID($widgettype)))
		{
			throw (new vB_Exception_Warning('Trying to get widgettype title from invalid widgettype \'' . htmlspecialchars($widgettype) . '\''));
		}
