...
define('FILE_VERSION', '4.0.7'); // this should match installsteps.php
define('SIMPLE_VERSION', '407'); // see vB_Datastore::check_options()
...
*
* @package	vBulletin
* @version	$Revision: 38417 $
* @date		$Date: 2010-08-19 12:39:15 -0700 (Thu, 19 Aug 2010) $
*/
class vB_Database
{
	/**
...
class vB_Input_Cleaner
{
	...
	*/
	function vB_Input_Cleaner(&$registry)
	{
		...
		$registry->reloadurl = $this->add_query($this->xss_clean(VB_URL_PATH), VB_URL_QUERY_RAW);

		// store the current script
		$registry->script = SCRIPT;

		// store the scriptpath
		$registry->scriptpath = $this->add_query($this->xss_clean(VB_URL_PATH), VB_URL_QUERY);
...
class vB_Template
{
	...
	*/
	...
	/**
	* Factory method to create the template object. Will choose the correct
	* template evaluation method automatically.
	* Any preregistered variables are also registered and cleared from
	* the preregister cache.
	*
	...
	*/
	...
	{
		// moved to the constructor so that subclasses can take advantage of it.
		// the global templates/variables need to be available before we can do any template stuff
		global $bootstrap;
	...
	*/
	 ...
	 *
	 * @param string $template_name				- The name of the template to register for
	...
	*/
	...
	{
		//avoid fetching the hook code multiple times -- its always going to be the same.
		//follow the method used for fetch_musername
		static $hook_code = false;
		if ($hook_code === false AND class_exists('vBulletinHook', false))
		{
				$hook_code = vBulletinHook::fetch_hook('template_register_var');
		}
		if ($hook_code)
		{
				eval($hook_code);
	 ...
	 */
	public function quickRegister($values)
	{
		if (!is_array($values))
		{
			return;
	...
	*                   If not specified, an array of all variables.
	*/
	...
	*/
	public function render($suppress_html_comments = false)
	{
		global $vbulletin, $vbphrase, $vbcollapse, $ad_location, $style;

		$bbuserinfo = $vbulletin->userinfo;
		$vboptions = $vbulletin->options;
		$session = $vbulletin->session->vars;
		$relpath = htmlspecialchars_uni($vbulletin->input->fetch_relpath());
		...
		//#29149 -- changed switch for show variable to array and added template_hook
		//these are important globals that always need to be available.  However, in some cases
		//we explicitly register a value that may be different from the global (which is the correct
		//way to do things, really).  In this case we need to ensure that we only use the global
		//when we don't have a local registered copy.
		$magic_globals = array('show', 'template_hook');
		foreach ($magic_globals as $magic_global)
		{
			if (!$this->is_registered($magic_global))
			{
				global $$magic_global;
			}
		}
...
class vB_Template_Runtime
{
	...
	public static function linkBuild($type, $info, $extra = array(), $primaryid = null, $primarytitle = null)
	{
