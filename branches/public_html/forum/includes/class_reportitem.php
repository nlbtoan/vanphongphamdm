...
class vB_ReportItem_VisitorMessage extends vB_ReportItem
{
	...
	function set_reportinfo(&$reportinfo)
	{
		$reportinfo = array_merge($reportinfo, array(
			...
			'pagetext'     => $this->iteminfo['pagetext'],
			'memberlink_pi'=> fetch_seo_url(
				'member|nosession',
				array(
					'userid' => $this->extrainfo['user']['userid'],
				 	'username' => unhtmlspecialchars($this->iteminfo['postusername'])
				),
				array(
...
class vB_ReportItem_PictureComment extends vB_ReportItem
{
	...
	function set_forminfo(&$iteminfo)
	{
		...
		$this->set_reporting_hidden_value('groupid', $this->extrainfo['group']['groupid']);
		$this->set_reporting_hidden_value('albumid', $this->extrainfo['album']['albumid']);
		$this->set_reporting_hidden_value('attachmentid', $this->extrainfo['picture']['attchmentid']);
	...
	function set_reportinfo(&$reportinfo)
	{
		$reportinfo = array_merge($reportinfo, array(
			...
			'commenturl' => ($this->extrainfo['picture']['albumid']
				? "album.php?" . $this->registry->session->vars['sessionurl'] . "albumid=" . $this->extrainfo['picture']['albumid'] . "&attachmentid={$this->iteminfo['attachmentid']}&commentid={$this->iteminfo['commentid']}#picturecomment{$this->iteminfo['commentid']}"
				: "group.php?" . $this->registry->session->vars['sessionurl'] . "do=picture&groupid=" . $this->extrainfo['group']['groupid'] . "&attachmentid={$this->iteminfo['attachmentid']}&commentid={$this->iteminfo['commentid']}#picturecomment{$this->iteminfo['commentid']}"
			),
			'pagetext'   => $this->iteminfo['pagetext'],
			'memberlink' => fetch_seo_url('member|nosession', array('userid' => $this->iteminfo['postuserid'], 'username' => unhtmlspecialchars($this->iteminfo['postusername']))),
		));
	}
