...
if ($_POST['do'] == 'addcat')
{
		...
		"))
		{
			...
			if ($vbulletin->GPC['dbutton'])
			{
				if ($vbulletin->GPC['delete'])
				{
					...
					$dataman->delete();
					$vbulletin->url = 'blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . 'do=editcat' . ($vbulletin->GPC['userid'] ? "&amp;u=$userinfo[userid]" : '');
					eval(print_standard_redirect('redirect_blog_profileupdate'));
				}
				else
				{
	...
	if (empty($errors) AND !defined('PREVIEW'))
	{
		...
		else
		{
			...
			$vbulletin->url = 'blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . 'do=editcat' . ($show['modedit'] ? "&amp;u=$userinfo[userid]" : '');
			eval(print_standard_redirect('redirect_blog_profileupdate'));
		}
	}
}
...
if ($_REQUEST['do'] == 'modifycat')
{
	...
	else
	{
		...
		if ($vbulletin->GPC['blogcategoryid'])
		{
			$navbits['blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . "do=editcat&amp;u=$userinfo[userid]"] = $vbphrase['blog_categories'];
...
if ($_REQUEST['do'] == 'stats')
{
	...
	$sorturl = 'blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . 'do=stats';
...
if ($_POST['do'] == 'updatesidebar')
{
	...
	if (!$vbulletin->GPC['addblock'])
	{
		if ($vbulletin->GPC['type'] == 'page')
		{
			...
			$vbulletin->url = 'blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . 'do=custompage';
			eval(print_standard_redirect('redirect_blog_profileupdate'));
		}
		else
		{
...
if ($_POST['do'] == 'deleteblock')
{
	...
	else
	{
		...
		{
			$vbulletin->url = 'blog.php?' . $vbulletin->session->vars['sessionurl'] . "u=$sidebarinfo[userid]";
		}
		else
		{
			$vbulletin->url = 'blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . 'do=custompage';
...
if ($_POST['do'] == 'adduser')
{
	...
	$vbulletin->url = 'blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . 'do=groups#member' . $userinfo['userid'];
...
if ($_REQUEST['do'] == 'groups')
{
	...
	)
	{
		...
		$members = $db->query_read_slave("
			...
			FROM " . TABLE_PREFIX . "blog_groupmembership AS gm
			LEFT JOIN " . TABLE_PREFIX . "user AS user on (user.userid = gm.userid)
...
if ($_REQUEST['do'] == 'userperm')
{
	...
	$navbits = array(
		'blog_usercp.php?' . $vbulletin->session->vars['sessionurl'] . 'do=groups' => $vbphrase['blog_groups'],
...
if ($_POST['do'] == 'docustomize')
{
	...
	{
		$usercss->save();
		$vbulletin->url = "blog_usercp.php?"  . $vbulletin->session->vars['sessionurl'] . "do=customize";
