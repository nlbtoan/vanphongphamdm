...
abstract class vBCms_Content extends vB_Content
{
	...
	public function setParameter($parameter, $value)
	{
		...
		{
			throw (new vB_Exception_Content('Attempting to set invalid parameter \'' . htmlspecialchars($parameter) . '\' on content handler \'' . $this->package . '_' . $this->class));
		}
	...
	public function setConfig($config, $value = null, $ignore_errors = false)
	{
		...
		else
		{
			...
			{
				throw (new vB_Exception_Content('Attempting to set invalid cvar \'' . htmlspecialchars($config) . '\' on content handler \'' . $this->package . '_' . $this->class));
			}
			else
			{
...
//		}
	 ...
	 */
	public function getPageURL($parameters = false)
	{
		$this->loadContent();

		return vBCms_Route_Content::getURL(array('node' => $this->getNodeURLSegment()));
	}
