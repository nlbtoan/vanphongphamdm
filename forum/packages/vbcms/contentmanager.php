...
class vBCms_ContentManager
{
		...
		foreach($sortable as $sorted_section)
		{
			foreach($sorted_section['parents'] AS $parent)
			{
				$thisline .= $sections[$parent]['title'] . ((vB_Template_Runtime::fetchStyleVar('textdirection') == 'ltr') ?
					'&gt;' : '&lt;') ;
				$stack[$count][] = htmlspecialchars_uni($sections[$parent]['title']);
			}

			$stack[$count][] = '<a href="javascript: setSection('
				. $sorted_section['nodeid'] . ",'"
				. vB_Template_Runtime::escapeJS(htmlspecialchars_uni($sorted_section['title'])) . "'); return false;\" onclick=\"javascript:void setSection("
				. $sorted_section['nodeid'] . ",'"
				. vB_Template_Runtime::escapeJS(htmlspecialchars_uni($sorted_section['title'])). "');return false;\" style=\"font-weight:bold;\">"
				. htmlspecialchars_uni($sorted_section['title'] ) . '</a>';
			$count++;
		}
	...
	*****/
	...
	public static function showCategories($nodeid, $per_page = 50, $page = 1)
	{
		...
		foreach ($data as $record)
		{
			...
			$category_name_display = str_repeat('&mdash;' , $record['cat_level'] * 1) . ' ' . htmlspecialchars_uni($record['category']);
			...
			<div style=\"float:" . vB_Template_Runtime::fetchStyleVar('right') . "\">
				...
				<a href=\"javascript:showCatEdit('edit',". $cat_edit_node_id. ', ' . $cat_edit_cat_id . ", '"
					.  vB_Template_Runtime::escapeJS(htmlspecialchars_uni($record['category'])) . "', '" .
	...
	*********/
	private static function getCategoryEditPanel()
	{
		...
		<div style=\"" . vB_Template_Runtime::fetchStyleVar('left') . ":0px;top:0px;position:absolute;text-align:" . vB_Template_Runtime::fetchStyleVar('left') . ";\"><strong>" .  $vbphrase['category_editor_section_navigator'] /* $vbphrase['edit_category'] */ . "</strong>
		</div><br />&nbsp;
		<div style=\"" . vB_Template_Runtime::fetchStyleVar('left') . ":50%;width:50%;top:0px;position:absolute;text-align:" . vB_Template_Runtime::fetchStyleVar('right') . ";\">
	...
	foreach ($sections AS $section)
	{
		...
		foreach ($section AS $value)
		{
			...
			if (is_array($value))
			{
				$section = htmlspecialchars_uni($value['section']);
				$path .= $section;
			...
			{
				$final_value = htmlspecialchars_uni($value);
				$path .= $final_value . '&nbsp;&gt;&nbsp;';
	 		}
	...
	private static function getSectionEditPanel()
	{
		...
		<div style=\"" . vB_Template_Runtime::fetchStyleVar('left') . ":50%;width:50%;top:0px;position:absolute;text-align:" . vB_Template_Runtime::fetchStyleVar('right') . ";\">
		<input type=\"button\" value=\"" . $vbphrase['save'] . "\" onclick=\"setEditedSection('" . htmlspecialchars_uni($vbphrase['need_title']) . " ');\">
		<input type=\"button\" value=\"" . $vbphrase['close'] .
...
//		}
	...
	***/
	public static function getTimeOffset($userinfo, $adjust_for_server = true)
	{
 ...
 //We are creating a new node. We'll just set its value at one larger than the largest node
	...
	************/
	 ...
	 ************/
	public function swapNodes($id1, $id2)
	{
...
/*				<td class=\"thead\">" . $vbphrase['layout'] . "</td>
	...
	*********************/
	...
	public static function getSections($withcontent = true, $all_nav = false)
	{
