...
function get_fbprofilepicurl()
{
	...
	// attempt pull profile pic from userinfo, cookie, or use unknown.gif, in that order
	if (empty($picurl))
	{
		$picurl = !empty($vbulletin->userinfo['fbuserid']) ? (Facebook::$DOMAIN_MAP['graph'].$vbulletin->userinfo['fbuserid'].'/picture') : $vbulletin->input->clean_gpc('c', COOKIE_PREFIX . 'fbprofilepicurl', TYPE_STR);
