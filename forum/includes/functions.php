...
function create_full_url($url = '')
{
	...
	if (!preg_match('#^[a-z]+(?<!about|javascript|vbscript|data)://#i', $url))
	{
		...
		else if (VB_URL_SCHEME)
		{
			...
			$url = VB_URL_BASE_PATH . ltrim($url, '/\\');
		}
	}
	else
	{
 ...
 * @return	string	Encoded string
 */
function ncrencode($str, $skip_ascii = false)
{
	if (!$str)
	{
		return $str;
	...
	if (is_pcre_unicode())
	{
		return preg_replace_callback(
			'#\X#u',
			create_function('$matches', 'return ncrencode_matches($matches, ' . (int)$skip_ascii . ');'),
			$str
		);
	}
