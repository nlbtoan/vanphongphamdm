...
if ($_POST['do'] == 'dodeletecomment')
{
	...
	while ($comment = $db->fetch_array($comments))
	{
		...
		if (($physicaldel AND !fetch_comment_perm('canremovecomments', $entryinfo, $comment)) OR (!$physicaldel AND !fetch_comment_perm('candeletecomments', $entryinfo, $comment)))
		{
	...
	foreach($commentarray AS $blogtextid => $comment)
	{
		...
		{
			$vbulletin->url = 'blog.php?' . $vbulletin->session->vars['sessionurl'] . 'b=' . $vbulletin->GPC['blogid'];
		}
	}
