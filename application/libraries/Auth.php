...
class MY_Auth
{
	...
	public function get_users(&$count, $group = false, $limit = false, $offset = false, $term = false) {
		return $this->ci->auth_model->get_users($count, $group, $limit, $offset, $term);
	}
	...
	public function __call($method, $arguments)
	{
		if( !method_exists( $this->ci->auth_model, $method) ) {
			show_error('Undefined method auth::' . $method . '() called');
		}
	...
	public function is_admin() {
		$admin_group = $this->ci->config->item('admin_group', 'auth');
	...
	public function logged_in() {
		$identity = $this->ci->config->item('identity', 'auth');
	...
	{
		$session  = $this->ci->config->item('identity', 'auth');
	...
	{
		$email_activation = $this->ci->config->item('email_activation', 'auth');

		if (!$email_activation)
		{
			if ($this->ci->auth_model->register($username, $password, $email, $additional_data, $group_name))
			{
		...
		else
		{
			...
			$deactivate = $this->ci->auth_model->deactivate($id);
			...
			$identity        = $this->ci->config->item('identity', 'auth');
			$user            = $this->ci->auth_model->get_user($id)->row();
			...
			$message = $this->ci->load->view($this->ci->config->item('email_templates', 'auth').$this->ci->config->item('email_activate', 'auth'), $data, true);
			...
			$this->ci->email->subject($this->ci->config->item('site_title', 'auth') . ' - Account Activation');
