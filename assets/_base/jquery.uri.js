var uri = {	
	...
	_fetch_uri_string : function() {		
		this.uri_string = this.url_to_uri(window.location + '');
		
		this.base_uri_string = window.location + '';		
		this.base_uri_string = this.base_uri_string.replace(/#.*/, '');
		this.base_uri_string = this.url_to_uri(this.base_uri_string);
	...
	url_to_uri : function(url) {
		...
		for (var i=split.length-1; i >=0 ; i --) {
			if (split[i] != '' && split[i] != '!') {						
				var uri = split[i].toLowerCase().substr(0); // bo dua ! dang dau
				// clean				
				//uri = hex.remove_suffix(uri);						
				// loai bo base_url
				var reg = new RegExp(BASE_URL + '?', 'i');
				if (reg.test(uri)) {					
					uri = uri.replace(reg, '');					
				}else{
					// khong co base_url --> not minde
					if (url.match(/^https?\/\//) ) {
						return false;
					}
				}
				// toi h moi duoc dung, khong no bien http:// --> http:/
				//uri = hex.trim(uri, '/');
				// xoa language
				//reg = new RegExp('/?'+hex.LANGUAGE.abbr+'/?', 'i');				
				//uri = uri.replace(reg, '');				
